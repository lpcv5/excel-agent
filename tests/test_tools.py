...
class TestGetExcelManager:
    ...
    def test_get_excel_manager_singleton(self):
        ...
        import tools
        if hasattr(tools.excel_tool._thread_local, 'manager'):
            delattr(tools.excel_tool._thread_local, 'manager')
    ...
    def test_get_excel_manager_creates_new_instance(self):
        ...
        assert tools.excel_tool._thread_local.manager is manager


class TestExcelStatus:
    """Tests for excel_status tool."""

    @patch("tools.excel_tool.get_excel_manager")
    ...
    @patch("tools.excel_tool.get_excel_manager", side_effect=Exception("Test error"))
...
class TestExcelOpenWorkbook:
    ...
    @patch("tools.excel_tool.get_excel_manager")
    @patch("libs.excel_com.workbook_ops.open_workbook")
    ...
    @patch("tools.excel_tool.get_excel_manager")
    @patch("libs.excel_com.workbook_ops.open_workbook", side_effect=FileNotFoundError("Not found"))
    ...
    @patch("tools.excel_tool.get_excel_manager")
    @patch("libs.excel_com.workbook_ops.open_workbook", side_effect=Exception("Test error"))
...
class TestExcelReadRange:
    ...
    @patch("tools.excel_tool.get_excel_manager")
    @patch("libs.excel_com.workbook_ops.read_range", side_effect=ValueError("Sheet not found"))
...
class TestExcelCloseWorkbook:
    ...
    @patch("tools.excel_tool.get_excel_manager")
    @patch("libs.excel_com.workbook_ops.close_workbook")
    ...
    @patch("tools.excel_tool.get_excel_manager")
    @patch("libs.excel_com.workbook_ops.close_workbook", side_effect=ValueError("Not open"))
...
class TestExcelToolsList:
    ...
    def test_tools_count(self):
        ...
        # Total: 1 status + 7 workbook + 5 worksheet + 4 format + 2 formula + 3 col/row = 22
        assert len(EXCEL_TOOLS) == 22
