...
class MessageParser:
    ...
    def __init__(self, track_tool_lifecycle: bool = True):
        ...
        self._seen_tool_calls: set[str] = (
            set()
        )  # Track tool calls we've already emitted
    ...
    def _parse_ai_message(
                    ...
                    if isinstance(item, dict):
                        ...
                        if item_type == "text":
                            ...
                            if text:  # Emit all text including whitespace
                                events.append(
                                    ContentEvent(content=text, node=node_name)
                                )
                        elif item_type == "reasoning":
                            reasoning = item.get("reasoning", "")
                            if reasoning:
                                events.append(
                                    ContentEvent(content=reasoning, node="thinking")
                                )
        ...
        if (
            self._track_tool_lifecycle
            and hasattr(message, "tool_call_chunks")
    ...
    def _parse_tool_call_chunk(
        ...
        if tool_name and tool_id:
            if tool_id not in self._seen_tool_calls:
                ...
                events.append(ToolCallStartEvent(id=tool_id, name=tool_name))
