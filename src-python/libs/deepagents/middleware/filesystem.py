...
from libs.deepagents.middleware._utils import append_to_system_message
...
- `*.txt` - Find all text files in the base directory
- `src\\**\\*.md` - Find all markdown files under src subdirectory"""
...
Use Windows absolute paths (e.g., D:\\path\\to\\file.txt or D:\\path\\to\\dir).

- ls: list files in a directory (requires Windows absolute path)
...
class FilesystemMiddleware(AgentMiddleware):
    ...
    Example:
        ...
        from libs.deepagents.backends import StateBackend, StoreBackend, CompositeBackend
    ...
    def _create_ls_tool(self) -> BaseTool:
        ...
        tool_description = (
            self._custom_tool_descriptions.get("ls") or LIST_FILES_TOOL_DESCRIPTION
        )
        ...
        def sync_ls(
            ...
            path: Annotated[
                str,
                "Windows absolute path to the directory to list (e.g., D:\\path\\to\\dir).",
            ],
        ) -> str:
        ...
        async def async_ls(
            ...
            resolved_backend = self._get_backend(runtime)
            infos = await resolved_backend.als_info(path)
            paths = [fi.get("path", "") for fi in infos]
            result = truncate_if_too_long(paths)
            return str(result)
    ...
    def _create_read_file_tool(self) -> BaseTool:
        ...
        def sync_read_file(
            file_path: Annotated[
                str,
                "Windows absolute path to the file to read (e.g., D:\\path\\to\\file.txt).",
            ],
            runtime: ToolRuntime[None, FilesystemState],
            offset: Annotated[
                int,
            ...
            if ext in IMAGE_EXTENSIONS:
                responses = resolved_backend.download_files([file_path])
                if responses and responses[0].content is not None:
                    media_type = IMAGE_MEDIA_TYPES.get(ext, "image/png")
                    image_b64 = base64.standard_b64encode(responses[0].content).decode(
                        "utf-8"
                    )
                    return ToolMessage(
                        content_blocks=[
            ...
            if token_limit and len(result) >= NUM_CHARS_PER_TOKEN * token_limit:
                ...
                max_content_length = NUM_CHARS_PER_TOKEN * token_limit - len(
                    truncation_msg
                )
        ...
        async def async_read_file(
            ...
            ext = Path(file_path).suffix.lower()
            if ext in IMAGE_EXTENSIONS:
                responses = await resolved_backend.adownload_files([file_path])
                if responses and responses[0].content is not None:
                    media_type = IMAGE_MEDIA_TYPES.get(ext, "image/png")
                    image_b64 = base64.standard_b64encode(responses[0].content).decode(
                        "utf-8"
    ...
    def _create_write_file_tool(self) -> BaseTool:
        ...
        def sync_write_file(
            file_path: Annotated[
                str,
                "Windows absolute path where the file should be created (e.g., D:\\path\\to\\file.txt).",
            ],
            content: Annotated[
                str,
    ...
    def _create_edit_file_tool(self) -> BaseTool:
        ...
        def sync_edit_file(
            file_path: Annotated[
                str,
                "Windows absolute path to the file to edit (e.g., D:\\path\\to\\file.txt).",
            ],
    ...
    def _create_glob_tool(self) -> BaseTool:
        ...
        tool_description = (
            self._custom_tool_descriptions.get("glob") or GLOB_TOOL_DESCRIPTION
        )

        def sync_glob(
            pattern: Annotated[
                str, "Glob pattern to match files (e.g., '**/*.py', '*.txt')."
            ],
            runtime: ToolRuntime[None, FilesystemState],
            path: Annotated[
                str,
                "Windows absolute base directory to search from (e.g., D:\\path\\to\\dir). Defaults to working directory.",
            ] = ".",
        ) -> str:
    ...
    def _create_grep_tool(self) -> BaseTool:
        ...
        tool_description = (
            self._custom_tool_descriptions.get("grep") or GREP_TOOL_DESCRIPTION
        )

        def sync_grep(
            pattern: Annotated[
                str, "Text pattern to search for (literal string, not regex)."
            ],
            runtime: ToolRuntime[None, FilesystemState],
            path: Annotated[
                str | None,
